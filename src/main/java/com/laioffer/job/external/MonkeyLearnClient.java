...
public class MonkeyLearnClient {
    ...
    private static final String AUTH_TOKEN = "4accd3e6dc86eed3fbbef23bbb04e10dd6ddeb30";

    /**
     * input List<String> is a list of "article", the Monkey Learn will deal with these articles independently
     * that's why the output is List of Set<String>
     *
     * Q: why not return Map<word, frequency> ???
     *
     * */
    public List<Set<String>> extract(List<String> articles) {
        ObjectMapper mapper = new ObjectMapper();
        CloseableHttpClient httpclient = HttpClients.createDefault();   // Apache client

        HttpPost request = new HttpPost(EXTRACT_URL);
        request.setHeader("Content-type", "application/json");
        request.setHeader("Authorization", "Token " + AUTH_TOKEN);  // we add a space after Token on purpose
        ...
        ResponseHandler<List<Set<String>>> responseHandler = response -> {
            ...
            //这里和Github中的请求是一样，使用Jackson中的ObjectMapper
