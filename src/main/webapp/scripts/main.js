(function () {
    ...
    function init() {
        //监听器
        //一定要先加监听器。再写validate这行，不然用户点了，还没加进去。
    ...
    function validateSession() {
        onSessionInvalid();
        var url = './login';    // 这里决定我们选哪个API (即：后端servlet)
    ...
    function showRegisterForm() {
        //先做选择器拿出来Obj，然后隐藏/显示，然后clear可能出现的遗留
        var loginForm = document.querySelector('#login-form');
        var registerForm = document.querySelector('#register-form');
        var itemNav = document.querySelector('#item-nav');
        var itemList = document.querySelector('#item-list');
    ...
    function register() {
        ...
        var firstName = document.querySelector('#register-first-name').value; // .value
        ...
        //正则表达式，只能由小写字母和数字组成，且必须大于1
        if (username.match(/^[a-z0-9_]+$/) === null) {
            showRegisterResult('Invalid username');
            return
        }

        // md5 是一种hash/encoding。md5是专门一个库
    ...
    /*
    * XMLHttpRequest 就是ajax，是js提供的一个frameword (class)
    *
    * **/
    function ajax(method, url, data, successCallback, errorCallback) {
        var xhr = new XMLHttpRequest();

        // 这里的method来告诉ajax，调用的是doPost还是doGet等...
    ...
    function login() {
        ...
        ajax('POST', url, req,
            ...
            function(res) {
                ...
                // successfully logged in
                if (result.status === 'OK') {
                    onSessionValid(result); // 登陆成功以后，要做的事情
                }
            },
